...
namespace SIGENCEScenarioTool.Extensions
{
    static public class XElementExtension
    {
        ...
        static public T GetProperty<T>(this XElement eParent, string strElementName, T tDefault)
        {
            ...
            //if (t.ImplementsInterface(typeof(IFromStringConverter<T>)))
            //{
            //    XElement x = eParent.Element(strElementName);

            //    if (x != null && x.Value.IsNotEmpty())
            //    {
            //        T value = (t as IFromStringConverter<T>).FromString(x.Value);
            //        return value;
            //    }
            //}

            //if (t is IFromStringConverter<T>)
            //{
            //    XElement x = eParent.Element(strElementName);
